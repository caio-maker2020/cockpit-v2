...
//
// Tipos suportados: cancelar_reentrega_ssw, executar_acao_autonoma.
//
// cobranca_email: REMOVIDO (Caio 2026-09-02, ADR 0020). O handler criava to-do
// "Reenviar cobrança" e, sem contato de e-mail, lançava erro e ficava pendente
// pra sempre (retry a cada 5 min: 14.542 CobrancaAdiadaSemContato em 10 dias).
// Qualquer linha desse tipo que ainda apareça é CANCELADA aqui, nunca
// processada. Guard: INV-137.
// =============================================================================
  ...
  for (const acao of (pendentes ?? []) as AcaoAgendada[]) {
    try {
      if (acao.tipo === "cobranca_email") {
        // Zumbi de antes de 2026-09-02: cancela em vez de processar. Nunca retenta.
        await supabase
          .from("acoes_agendadas")
          .update({
            status: "cancelado",
            cancelado_motivo: "Caio 2026-09-02: cobrança automática removida (ADR 0020) — cancelamento defensivo no processador",
            processed_at: new Date().toISOString(),
          })
          .eq("id", acao.id);
        await supabase.from("card_events").insert({
          card_id: acao.card_id,
          event_type: "CobrancaAutomaticaCancelada",
          actor_type: "system",
          actor_id: "processar-acoes-agendadas",
          payload: { acao_id: acao.id, motivo: "cobrança automática removida 2026-09-02 (ADR 0020)" },
        });
        summary.processados++;

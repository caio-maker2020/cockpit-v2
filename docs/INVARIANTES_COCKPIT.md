...
- 2026-09-19 — **INV-156 adicionado após o incidente mais silencioso da história do projeto: 24 HORAS DE COCKPIT MORTO SEM NENHUM ALARME.** O agendador interno do banco (pg_cron) parou de disparar em 18/09 ~13:16-13:22 UTC — minutos após a aplicação da mig 404 (fundação da memória do card, que criou o job de 1min `atualizar-estado-tratativa`; correlação temporal fortíssima, mecanismo interno NÃO confirmado porque o pipeline de logs congelou JUNTO: `postgres_logs` reteve apenas 15 linhas num intervalo de 22 segundos, todas às 13:22, mostrando o cron ainda disparando — depois, silêncio total também nos logs). Efeito: zero card_events desde 18/09 13h, zero e-mails processados, zero chamadas de IA, sync/triagem/agentes/trilho parados — enquanto o front respondia normalmente e o status da Supabase dizia ACTIVE_HEALTHY (o health-check só olha o postmaster). Descoberto POR ACASO ~24h depois, ao investigar por que a re-análise da NF 138102 nunca chegou. Sinais ignorados na hora (lição de comportamento): um vigia expirou sem resultado e não foi investigado; 7 ações do trilho vencidas há horas "pendentes" idem. Fix imediato: restart da instância via Management API (`POST /v1/projects/<ref>/restart` — SEM perda de dados; o que não entrou ficou retido no Gmail/Postmark/Bastão e drenou depois). Proteções permanentes: (a) watchdog EXTERNO `.github/workflows/pulso-cockpit.yml` (GitHub Actions, */10min, secret SUPABASE_DB_URL via pooler IPv4) — falha do workflow = e-mail automático pro dono do repo; NUNCA remover; (b) regra no CLAUDE.md: migration que toca cron/trigger/worker só está concluída com PROVA DE PULSO (max(start_time) avançando em 10min); (c) checagem de pulso no /verify-cockpit. REGRA INVIOLÁVEL: o pulso do sistema é vigiado de FORA do sistema; silêncio total nunca é "dia calmo" — é o sintoma máximo; e nenhuma mudança em agendamento se dá por encerrada sem ver o relógio bater depois dela.

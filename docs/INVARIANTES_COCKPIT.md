...
## INV-042 — Resposta REAL de cliente NUNCA é muda: card terminal REABRE

**Regra.** Resposta real de cliente (não-bounce; filtro no gmail-poll, NF 5826) vinculada a card em `TRANSFERIDO`/`RESOLVIDO` **reabre** o card: AVH + lock + `cliente_respondeu_em` + propostas pós-resposta + interpretador. Fonte única da decisão: `decidirAcionamentoPorRespostaCliente` (`_shared/acionamento-resposta-cliente.ts`) — usada nos DOIS caminhos do vinculador (thread e NF). Nunca reimplementar a decisão inline (foi a divergência entre os 2 caminhos que criou o buraco). `EXTRAVIO_MONITORADO`/`CANCELADO` ficam fora deliberadamente (extravio tem reconciliação própria, INV-017). Evento de auditoria: `CardReabertoPorRespostaCliente`. 3ª camada: watchdog `checkRespostaClienteEngolida` no health-check (e-mail ≤2h). Estende o INV-016: "cliente respondeu → SEMPRE visível com as ações" — a palavra do cliente vale mais que a verdade do Bastão.

**Guard:** INV-042 no verify-cockpit (fonte única + uso ≥3 no vinculador + `acionamento-resposta-cliente.test.ts` + watchdog ≥2 + SQL "nenhuma resposta engolida em 24h").

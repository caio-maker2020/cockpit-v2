...
**Regra.** Toda ação em `acoes_agendadas` que falha OU avança `executar_em` pro futuro OU muda pra estado ≠ `pendente`. Proibido falhar e manter `executar_em` no passado — a janela do `processar-acoes-agendadas` é `ORDER BY executar_em ASC LIMIT 200`, então pendência eterna ocupa vaga pra sempre e starva tudo que vence depois (inclusive o "Forçar agora", que recoloca a ação com hora = agora, atrás das presas). Corolário: **cobrança automática foi APAGADA** (Caio/Matheus 2026-07-17, mig 298 — sem flag): NENHUM código cria `cobranca_email` (RPC `agendar_cobranca_email` dropado; portas do executor/enviar-resposta/`marcar_retorno_inconclusivo` removidas). Religar exige migration nova + ADR. O handler de `cobranca_email` fica como dreno defensivo de stragglers.

**Onde vive.** Decisor puro `_shared/fila-acoes-agendadas.ts` (`decidirProximoPassoFalhaCobranca`: reagenda +24h com `payload.tentativas++`, teto 5 → `falha_definitiva` = status `cancelado` + alert `cobranca_falha_definitiva` em `alerts`; evento `CobrancaAdiadaSem*` só na 1ª falha). NÃO usar `precisa_acao` pra `cobranca_email` — todos os consumidores desse status filtram `tipo='cancelar_reentrega_ssw'` (ação ficaria invisível e o `cancelar_acoes_agendadas_do_card` nunca a limparia). Usado pelo handler `processarCobrancaEmail` em `processar-acoes-agendadas/index.ts` (controla o próprio status; UPDATEs checam `{error}` e têm guard `status='pendente'`; exceções caem num best-effort de INV-fila no catch do loop; summary separa `processados` de `cobrancas_reagendadas`/`cobrancas_falha_definitiva`). Alerta de saúde da fila no `audit-invariante` (vencidas ≥150 ou mais velha >2h → alert `fila_acoes_agendadas_saturada`, cooldown 6h).

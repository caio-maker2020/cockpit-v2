-- ============================================================================
-- Cockpit v2 — RPC `lancar_oc_emergencial_acao_executada` v4
-- Data: 2026-05-11
--
-- Caio 2026-05-11: adiciona terceiro caso ao gate da RPC pra suportar o botão
-- "RECUSAR FLOW SUGERIDO" no front. Cenário real: operação SSW lança oc=20
-- (extravio localizado) indevidamente; cliente recebe notificação automática;
-- Larissa precisa corrigir lançando uma oc qualquer do catálogo (texto +
-- anexo opcional) sem esperar a operação refazer.
--
-- Gate combinado v4:
--   1. state = 'ACAO_EXECUTADA' (lançamento emergencial enquanto Bastão não confirma)
--   2. state = 'AGUARDANDO_AGENTE' sem propostas pendentes (oc sem regra mapeada)
--   3. state = 'AGUARDANDO_VALIDACAO_HUMANA' AND cod_ultima_ocorrencia = 20
--      (recusa do flow sugerido — correção de erro da operação)
--
-- Anti-presunção: a lista é hardcoded. Adicionar outras combinações exige
-- migration nova com aval do Caio (memory: "Nunca presumir sobre ocorrências
-- SSW — regras de oc só vêm do Caio; jamais inferir").
-- ============================================================================
...
COMMENT ON FUNCTION public.lancar_oc_emergencial_acao_executada(uuid, int, text, uuid) IS
  'Caio 2026-05-11 v4: lança oc emergencial em 3 cenários: '
  '(1) ACAO_EXECUTADA, '
  '(2) AGUARDANDO_AGENTE sem propostas pendentes, '
  '(3) AGUARDANDO_VALIDACAO_HUMANA com oc=20 (botão RECUSAR FLOW SUGERIDO — '
  'correção de erro da operação que lançou extravio localizado indevidamente). '
  'Opcional: texto livre + anexo JPEG/PDF (vira `imagem` base64 no SSW).';

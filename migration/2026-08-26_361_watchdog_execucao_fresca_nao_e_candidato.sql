-- =============================================================================
-- 2026-08-26_361 — watchdog execucao-presa: card com execucao FRESCA nao e
--                  candidato (falso-positivo por residuo de todo velho)
-- =============================================================================
-- Bug 3 de 26/08 (independente do poll/INV-109 e da starvation): o candidato
-- do reconciliador da mig 279 (emendado na 294) e "card EXECUTANDO_ACAO x
-- QUALQUER todo aprovado/executando com approved_at > threshold". Ele NAO
-- amarra o todo a execucao ATUAL. Prod tem 1.674 todos aprovado/executando
-- velhos em 939 cards vivos (fluxos que nao finalizam o status do todo — ex.:
-- a 49 do agente de extravio das 08:01 executou/confirmou 08:13 e o todo
-- continuou 'executando'). Qualquer card desses que entre em EXECUTANDO_ACAO
-- e cruze com o cron do watchdog leva revert em execucao saudavel.
--
-- Fix na RAIZ do falso-positivo: se o card tem todo aprovado/executando
-- FRESCO (< p_threshold_min), ele esta executando agora — pula. O proposito
-- legitimo fica intacto: card genuinamente preso nao tem execucao fresca.
--
-- Base: definicao ATUAL de producao (pg_get_functiondef 26/08, ja com o teto
-- de 24h da mig 294) + a cerca nova. _reconciliar_decidir intocada.
-- Higiene dos 1.674 todos velhos: decisao a parte (nao nesta mig).
-- Guard anti-regressao: INV-110. Idempotente. Sem BEGIN/COMMIT interno.
-- skill: supabase-postgres-best-practices (SECURITY DEFINER preservado com
-- search_path fixo, como ja estava).
-- =============================================================================

-- =============================================================================
-- 2026-08-31_370 — casos do time na 49: templates + trava do feedback ampliada
-- =============================================================================
-- Respostas oficiais do Caio 31/08 (memoria regras-caio-agente-49-respostas):
--   1. Template TENTATIVAS_ESGOTADAS (P1) — texto ditado pelo Caio;
--   2. Template CUSTO_ENTREGA_DEDICADO (P2) — valor+motivo+autorizacao;
--   3. Trava do feedback obrigatorio (mig 363) passa a cobrir TAMBEM o caso
--      'carona_pos54' (P3: 49 de anexo com cliente ja notificado no ciclo —
--      agente nao sugere nada e a explicacao do operador e obrigatoria).
-- Base da funcao = definicao ATUAL de prod. Idempotente. Sem BEGIN/COMMIT.
-- =============================================================================

INSERT INTO public.templates_email (id, nome, descricao, assunto, corpo_template, ativo)
VALUES
 ('TENTATIVAS_ESGOTADAS',
  'Tentativas de entrega esgotadas',
  '49 com 3 tentativas confirmadas (>=3 oc 14 no historico) — pede orientacao ao pagador',
  '[Sal Express] NF {nf} — 3 tentativas de entrega sem sucesso',
  E'Prezado(a),\n\nRealizamos 3 tentativas de entrega da NF {nf} que nao foram bem-sucedidas.\n\nPrecisamos de sua orientacao sobre como proceder: havera algum novo detalhe (endereco, contato, agendamento) para finalizarmos a entrega, ou ja podemos seguir com a devolucao?\n\nFicamos no aguardo do seu retorno.\n\nAtenciosamente,\nSal Express — Relacionamento',
  true),
 ('CUSTO_ENTREGA_DEDICADO',
  'Autorizacao de custo extra de entrega',
  '49 sinalizando custo adicional/veiculo dedicado — repassa valor e pede autorizacao (NUNCA para OVD/FG)',
  '[Sal Express] NF {nf} — custo adicional de entrega: autorizacao',
  E'Prezado(a),\n\nPara concluir a entrega da NF {nf} sera necessario um custo adicional de transporte.\n\n{custo_detalhe}\n\nPodemos seguir com a cobranca deste valor para realizar a entrega? Com a sua autorizacao expressa, seguiremos imediatamente e o CT-e complementar sera emitido.\n\nFicamos no aguardo.\n\nAtenciosamente,\nSal Express — Relacionamento',
  true)

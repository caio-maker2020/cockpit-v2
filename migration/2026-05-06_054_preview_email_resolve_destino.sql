-- ============================================================================
-- Cockpit v2 — preview_email_todo: resolve email_destino mesmo quando todo
-- foi criado em modo sem_email
-- Data: 2026-05-06
--
-- Bug encontrado em testes: cards criados quando template estava inativo
-- (ex: NF 2148226) têm proposta_payload.args.email_destino=null. Modal
-- recebia null e Larissa não tinha pra quem enviar — mas cliente TEM email
-- cadastrado em contatos_cliente, basta resolver.
--
-- Fix: quando args.email_destino é null e não tem extras.email_destinatarios,
-- chama resolver_email_cobranca_cliente(cnpj_pagador, 'logistico') pra puxar
-- email do contato.
-- ============================================================================
...
BEGIN
  ...
  -- Email destino: extras.email_destinatarios[0] > args.email_destino >
  -- fallback resolver_email_cobranca_cliente (cliente cadastrado em
  -- contatos_cliente). Sempre tenta resolver — Larissa pode editar no modal.
